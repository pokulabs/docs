...
Poku forms let your AI agent collect structured data from a recipient mid-conversation or mid-task. The agent sends a unique Poku form link, the recipient fills out the form, and the response comes back to the agent — no separate form builder, webhook, or database required.

---

## How it works

1. Your agent sends a form via SMS, WhatsApp, or Slack
...
|---|---|---|
| `waitTime` | 90s | Holds the HTTP request open. Shown as a countdown on the form. |
...
Both `waitTime` and `followUpTime` must be supplied — at least one is required.
...
| **Customizable message** | Yes — defaults to `"Please fill out this form: {{url}}"` | No — message is fixed | Yes |
