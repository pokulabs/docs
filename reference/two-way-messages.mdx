...
<Steps>
  ...
  <Step title="followUpTime keeps the channel open">
    After `waitTime` closes, `followUpTime` continues listening for late replies and delivers them to your configured webhook. (Link to followUptime concept)
  </Step>
</Steps>
...
| **Template when `waitTime` is set** | Free-form | *"Your AI agent is requesting your input. Message from agent: \{\{message\}\}..."* | Free-form |
| **Requirement** | Using your own number requires messaging approval (e.g., A2P10DLC) | Poku WhatsApp or own number with Meta template | Recipient must be in the Poku workspace or install Pokubot to their workspace |
